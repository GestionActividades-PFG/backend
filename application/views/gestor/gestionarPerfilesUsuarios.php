...
<div class="container caja">
    ...
    <div class="row">
        ...
        <article class="col-md-9 articulo">
            ...
            <?php
                $sql_per_pro = "SELECT profesores.usuario, perfiles.nombrePerfil, profesores.idUsuario, perfiles.idPerfil
                                        FROM perfiles INNER JOIN perfiles_profesor
                                            ON perfiles_profesor.idPerfil=perfiles.idPerfil
                                        INNER JOIN profesores
                                            ON profesores.idUsuario=perfiles_profesor.idUsuario";
                $query = $this->db->query($sql_per_pro);
                ...
                if($query->num_rows()){
                    ...
                    foreach($query->result() as $fila){
                        ...
                        echo '<td><a href="'.base_url().'index.php/Gestor/CambiarPerfilUsuario?usuario='.$fila->idUsuario.'&perfil='.$fila->idPerfil.'">Modificar</a></td>';
                        echo '<td><a href="'.base_url().'index.php/Gestor/BorrarUsuario?usuario='.$fila->idUsuario.'&perfil='.$fila->idPerfil.'">Borrar</a></td>';
                        echo '</tr>';
                    }
                    echo '</tbody>';
                    echo '</table>';
            ...
            ?>
            <a href="<?php echo base_url()?>index.php/Gestor/AsignarPerfilUsuario" class="btn btn-success">Asignar perfiles a un usuario</a>
        </article>
    </div>
    <!-- /CUERPO DE LA PÁGINA -->
</div>
